...
<?php
$filelib = isset($_POST['flr']) ? $_POST['flr'] : '';
if (empty($filelib) && isset($_GET['flr'])) $filelib = $_GET['flr'];
...
$file = isset($_POST['filelist']) ? $_POST['filelist'] : '';

$preview = '';

$errors = array();
if (isset($_FILES['file_file']['size']) && $_FILES['file_file']['size'] > 0)
	if ($file = uploadFile('file_file')) $preview = $spaw_base_url.$filelib.$file;

// delete
if ($spaw_file_delete_allowed && isset($_POST['lib_action']) && ($_POST['lib_action'] == 'delete') && !empty($file))
	deleteFile();
?>
...
function uploadFile($file) {
  ...
  $image_name = upload::upload_file( $_FILES[$file], $_root.$filelib, false, true );
