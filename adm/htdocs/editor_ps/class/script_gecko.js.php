 ...
 /***********************/
  ...
  function SPAW_image_insert_click(editor, sender)
  {
    var wnd = window.open('<?php echo $spaw_dir?>dialogs/img_library.php?lang=' 
      ...
      'status=no,modal=yes,scrollbars=yes,resizable=yes,width=750,height=500'); 
  }
  
  function SPAW_image_insert_click_callback(editor, sender)
  {
    var iProps = sender.returnValue;
      ...
      if (iProps.height) 
      	newImg.height = iProps.height;     	
			...
			insertNodeAtSelection(ed.contentWindow, newImg);
		}
    ed.contentWindow.focus();
  }
  ...
  function SPAW_image_prop_click(editor, sender)
  {
    ...
    if (im)
    {
      ...
      var wnd = window.open('<?php echo $spaw_dir?>dialogs/img.php?lang=' 
        ...
        'status=no,modal=yes,width=750,height=500'); 
  ...
  function SPAW_file_insert_click(editor, sender) {
    var wnd = window.open('<?php echo $spaw_dir?>dialogs/file_library.php?lang=' 
      + document.getElementById('SPAW_'+editor+'_lang').value + '&theme=' 
      + document.getElementById('SPAW_'+editor+'_theme').value
      + '&editor=' + editor + '&callback=SPAW_file_insert_click_callback', "file_library", 
      'status=no,modal=yes,scrollbars=yes,resizable=yes,width=750,height=500'); 
  }
  //
  function SPAW_file_insert_click_callback(editor, sender) {
	var fileLink = sender.returnValue;
	...
	if (fileLink) {
		...
		if (ed.contentWindow.getSelection().rangeCount > 0 &&
			...
			// && fileLink.attributes["title"].nodeValue != ''
			a.innerHTML = fileLink.title ? fileLink.title : fileLink.href;
		}
