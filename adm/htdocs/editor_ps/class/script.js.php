...
SPAW_toggle_borders(editor,this[editor+'_rEdit'].document.body,null);
  ...
  */
  ...
  function SPAW_image_insert_click(editor, sender)
  {
    ...
    var iProps = showModalDialog('<?php echo $spaw_dir?>dialogs/img_library.php?lang=' + document.all['SPAW_'+editor+'_lang'].value + '&theme=' + document.all['SPAW_'+editor+'_theme'].value+'&request_uri='+escape(window.location.href), '',
      'dialogHeight:500px; dialogWidth:750px; scrollbars:yes; resizable:yes; status:no');
    ...
    if (iProps)
    	...
    	newImg.src = iProps.src;
    	newImg.border = iProps.border;
      if (iProps.width) 
      	newImg.width = iProps.width;
      if (iProps.height) 
      	newImg.height = iProps.height;     	

      var selection = window.frames[editor+'_rEdit'].document.selection.createRange();
      selection.pasteHTML(newImg.outerHTML);
		}
  ...
  function SPAW_image_popup_click(editor, sender)
  {
    ...
    var imgSrc = showModalDialog('<?php echo $spaw_dir?>dialogs/img_library.php?lang=' + document.all['SPAW_'+editor+'_lang'].value + '&theme=' + document.all['SPAW_'+editor+'_theme'].value+'&request_uri='+escape(window.location.href), '',
      'dialogHeight:500px; dialogWidth:750px; resizable:no; status:no');
  ...
  // FILE LINK INSERT!
  function SPAW_file_insert_click(editor, sender) {
	window.frames[editor+'_rEdit'].focus();

	var fileLink = {};

	var fileLink =  showModalDialog('<?php echo $spaw_dir?>dialogs/file_library.php?lang=' + document.all['SPAW_'+editor+'_lang'].value + '&theme=' + document.all['SPAW_'+editor+'_theme'].value+'&request_uri='+escape(window.location.href), '',
					'dialogHeight:500px; dialogWidth:500px; scrollbars:yes; resizable:yes; status:no');

	if (fileLink) {
		var a;
		a = document.createElement('A');

		if (fileLink.href) a.href = fileLink.href;

		if (window.frames[editor+'_rEdit'].document.selection.type == "Control") {
			var selection = window.frames[editor+'_rEdit'].document.selection.createRange();
//			a.innerHTML = selection(0).outerHTML;
			a.innerHTML = (fileLink.title && fileLink.title != '') ? fileLink.title : fileLink.href;
			selection(0).outerHTML = a.outerHTML;
		}
		else {
			var selection = window.frames[editor+'_rEdit'].document.selection.createRange();
//			if (selection.htmlText == '') 
			a.innerHTML = (fileLink.title && fileLink.title != '') ? fileLink.title : fileLink.href;
//			else a.innerHTML = selection.htmlText;
			selection.pasteHTML(a.outerHTML);
		}
	}
  ...
  function SPAW_update_toolbar(editor, force)
  {
    ...
    // enable file insert
    SPAW_toggle_tbi(editor,"file_insert", true);
    // enable table insert
    ...
    if (ct)
    {
      ...
      if (cr)
      {
        ...
        else
        {
          ...
          // disable file insert
          SPAW_toggle_tbi(editor,"file_insert", false);
        }
      }
      else
      {
