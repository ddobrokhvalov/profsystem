...
<form action="" method="post" onsubmit="return CheckForm.validate( this )">
	<table border="1">
		...
		<tr>
			<td>
				{$sysw_client_surname}: <span style="color: red">*</span>
			</td>
			<td>
				<input type="text" name="SURNAME_{$env.area_id}" value="{$SURNAME}" lang="errors_nonempty_">
			</td>
		</tr>
		<tr>
			<td>
		...
		<tr>
			<td>
				{$sysw_client_email} <span style="color: red">*</span>:
			</td>
			<td>
...
{else}
		<tr>
			<td>
				{$sysw_client_person}: <span class="red">*</span>
			</td>
			<td>
				<select name="CLIENT_TYPE_{$env.area_id}" lang="errors_nonempty_">
					<option value="0">{$sysw_client_person_physical}</option>
					<option value="1">{$sysw_client_person_legal}</option>
				</select>
			</td>
		</tr>
{/if}
